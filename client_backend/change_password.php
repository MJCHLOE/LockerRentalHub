...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($newPassword !== $confirmPassword) {
        echo "<script>alert('New passwords do not match.'); window.location.href='../../home.php';</script>";
        exit();
    }

    if (strlen($newPassword) < 6) {
        echo "<script>alert('New password must be at least 6 characters.'); window.location.href='../../home.php';</script>";
        exit();
    }
    ...
    if (!$storedHash || !password_verify($currentPassword, $storedHash)) {
        echo "<script>alert('Current password is incorrect.'); window.location.href='../../home.php';</script>";
        exit();
    }
